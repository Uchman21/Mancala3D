...
public class movehandprefab : NetworkBehaviour {
		...
		{
			startp = GameObject.FindGameObjectWithTag("startpoint");
			moveTime = 0.01f;
			inverseMoveTime = 2f / moveTime;
		Debug.Log (GetComponent<NetworkIdentity> ().isServer);
		if (GetComponent<NetworkIdentity> ().isServer) {
			startPoint = GameObject.FindGameObjectWithTag("startPos1");
			tag = "Player1";
			//mainCamera = GameObject.FindGameObjectWithTag("Cam1");
			if(GetComponent<NetworkIdentity> ().isLocalPlayer == true) transform.position = startPoint.transform.position;
			if(GetComponent<NetworkIdentity> ().isLocalPlayer == true) transform.rotation = startPoint.transform.rotation;
			if(GetComponent<NetworkIdentity> ().isLocalPlayer == true) StartCoroutine( movement());
		} else {
			startPoint = GameObject.FindGameObjectWithTag("startPos2");
			tag = "Player2";
			if(GetComponent<NetworkIdentity> ().isLocalPlayer == true) transform.position = startPoint.transform.position;
			if(GetComponent<NetworkIdentity> ().isLocalPlayer == true) transform.rotation = startPoint.transform.rotation;
		}
		...
		while (h <= 12) {
			if (i < 6) {
				...
				yield return StartCoroutine(Move ((int)startp.transform.position.x + (i * 10), (int)startp.transform.position.y + 2, (int)startp.transform.position.z, h));
				i = i+1;
			} else {
				yield return StartCoroutine( Move ((int)startp.transform.position.x + (j * 10), (int)startp.transform.position.y + 2, (int)startp.transform.position.z + 8, h));
				j = j - 1;
			}
			h = h + 1;
		}
	...
	public IEnumerator Move (int xDir, int yDir, int zDir, int num)
		...
		if (num > 0) {
			...
			NetworkServer.Spawn (points.gameObject);
		}
	...
	void Update () {
		if(GetComponent<NetworkIdentity> ().isLocalPlayer == false) return; // Don't check input if this isn't local
	}
